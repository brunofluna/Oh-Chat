...
def main(page: ft.Page):
    ...
    def entrar(e):
        if not nome_usuario.value:
            nome_usuario.error_text = "Tá equecendo de digitar o nome de usuário, não pode ficar em branco, seu abençoado!!!"
            nome_usuario.update()
        else:
            page.session.set("nome_usuario", nome_usuario.value)
            page.dialog.open = False
            page.pubsub.send_all(Mensagem(usuario=nome_usuario.value, texto=f'{nome_usuario.value} entrou no Oh chat!', tipo_msg="login_message"))
            page.update()
